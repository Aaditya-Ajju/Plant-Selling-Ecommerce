    ...
    return new Promise(
        async (resolve, rejected) => {
            try {
                ...
                // Always include Authorization header if accessToken exists
                if (accessToken) {
                    headers.Authorization = `Bearer ${accessToken}`;
                }

                // Conditionally add orderToken for checkout related paths
                if (path.startsWith("/api/v2/checkout") || orderToken) {
                    if (orderToken) {
                        if (headers.Authorization) {
                            headers.Authorization += ` orderToken ${orderToken}`;
                        } else {
                            // This case should ideally not happen if accessToken is handled above
                            headers.Authorization = `Bearer orderToken ${orderToken}`;
                        }
                    }
                }
                ...
                const res = await fetch(apiUrl, {
                    method,
                    headers: headers, // Use the dynamically built headers object
                    credentials: 'include',
                    body: body ? JSON.stringify(body) : null
                });
