...
exports.signIn = async (req, res, next) => {
    try {
        ...
        if (!token.refreshToken || !token.refreshToken.encryptedMessage || !token.refreshToken.iv) {
        ...
        await setData('authentication', token.refreshToken.encryptedMessage, 'refreshToken', { iv: token.refreshToken.iv }, 604800); // 7d = 7 * 24 * 60 * 60
        ...
        const info = {
            ...
            token: {
                ...
                refreshToken: token.refreshToken.encryptedMessage
            }
        }
...
exports.refreshToken = async (req, res, next) => {
    try {
        ...
        // Decrypt the refresh token IV
        const oldRefreshTokenIv = await getData('authentication', refreshToken, 'refreshToken');

        if (!oldRefreshTokenIv || !oldRefreshTokenIv.iv) {
            const error = new Error("Refresh token not found or invalid.");
            error.statusCode = 403;
            throw error;
        }

        const decryptedToken = decryptMessage(refreshToken, oldRefreshTokenIv.iv);

        // Invalidate the old refresh token from Redis
        ...
        const verifyUser = jwt.verify(decryptedToken, refreshSecret);
        ...
        const { encryptedMessage, newRefreshTokenIv } = token.refreshToken;

        await setData("authentication", encryptedMessage, "refreshToken", { iv: newRefreshTokenIv }, 604800); // 7d = 7 * 24 * 60 * 60
...
exports.validateOtp = async (req, res, next) => {
    ...
    try {
        ...
        const { encryptedMessage, refreshTokenIv } = authToken.refreshToken;

        if (!encryptedMessage || !refreshTokenIv) {
            const error = new Error("Failed to generate refresh token");
            error.statusCode = 500;
            throw error;
        }
        ...
        //* Save the IV in redis database with expire time 7.
        await setData('authentication', encryptedMessage, 'refreshToken', { iv: refreshTokenIv }, 604800); // 7d = 7 * 24 * 60 * 60
